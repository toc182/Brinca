...
import {
  ...
  uploadDrillResultPhoto,
} from '@/lib/sync/photo-upload-queue';
import {
  type DrillResultPhotoRow,
  deletePhoto,
  getPhotosByDrillResult,
  insertLocalPhoto,
  markPhotoPending,
} from '../repositories/drill-result-photo.repository';

export const MAX_PHOTOS_PER_DRILL = 10;

/**
 * `'draft'` is only used by editors that hold photos in component state
 * before the parent row exists (e.g. CreateDrillScreen's draft description
 * photos). PhotoGallery renders them without any status badge — they look
 * like a normal thumbnail with the optional remove (X) badge.
 */
export interface PhotoItem {
  id: string;
  uri: string;
  status: 'draft' | 'pending' | 'uploaded' | 'failed';
}
 ...
 */
async function withSignedUrls(rows: DrillResultPhotoRow[]): Promise<PhotoItem[]> {
...
const photosKey = (drillResultId: string) => ['drill-photos', drillResultId] as const;
...
export function useDrillPhotos(drillResultId: string | null) {
  const queryClient = useQueryClient();

  const query = useQuery({
    queryKey: photosKey(drillResultId ?? ''),
    queryFn: async () => {
      const rows = await getPhotosByDrillResult(drillResultId!);
      return withSignedUrls(rows);
    },
    enabled: !!drillResultId,
  });

  const photos: PhotoItem[] = query.data ?? [];
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: photosKey(drillResultId ?? '') });

  const addPhoto = useCallback(async () => {
    if (!drillResultId) return;
    if (photos.length >= MAX_PHOTOS_PER_DRILL) {
      showToast('warning', `Up to ${MAX_PHOTOS_PER_DRILL} photos per drill.`);
      return;
    }
    try {
      ...
      const row = await insertLocalPhoto(drillResultId, localUri);
      await invalidate();
      ...
      void uploadDrillResultPhoto(row).then(() => invalidate());
    } catch {
      showToast('error', 'Could not add photo. Try again.');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [drillResultId, photos.length]);
