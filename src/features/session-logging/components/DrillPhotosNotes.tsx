...
import { MAX_PHOTOS_PER_DRILL, useDrillPhotos } from '../hooks/useDrillPhotos';
import { PhotoGallery } from './PhotoGallery';

interface DrillPhotosNotesProps {
  drillResultId: string | null;
  note: string;
...
/**
 * Multi-photo + single-note section for the DrillScreen. Mirrors the visual
 * pattern of SessionNotes (two-card row) but adds a thumbnail strip below
 * for the multi-photo support.
 */
export function DrillPhotosNotes({
  drillResultId,
  note,
  onChangeNote,
  title = 'Drill note',
  placeholder = 'Anything notable from this drill?',
}: DrillPhotosNotesProps) {
  const { photos, addPhoto, removePhoto, retryUpload } = useDrillPhotos(drillResultId);
  ...
  const atCap = photos.length >= MAX_PHOTOS_PER_DRILL;
